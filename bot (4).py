...
async def reset_cmd(update: Update, context):
    ...
    await update.message.reply_text(
        "Профиль и история диалога удалены. Напишите /start чтобы начать заново."
    )
...
async def handler(update: Update, context):
    ...
    if q.data == "fill_profile":
        ...
        await q.message.reply_text(
            "Отлично! Заполним ваш профиль — займёт 1 минуту и поможет давать персональные советы.\n\nКак вас зовут?"
        )
        return
    ...
    if q.data == "show_profile":
        ...
        text += "История диалога сохранена и не потеряется при перезапуске."
    ...
    if q.data == "clear_history":
        clear_history(user_id)
        await q.message.reply_text("История диалога очищена. Анастасия начнёт разговор заново.")
        return
...
async def chat(update: Update, context):
    ...
    if user_id in PROFILE_STATE:
        ...
        if step == "age":
            ...
            await update.message.reply_text("Сколько лет вы работаете и в какой сфере? (например: 3 года в маркетинге)")
            return
        ...
        if step == "location":
            ...
            del PROFILE_DATA[user_id]
            await update.message.reply_text(
                "Профиль сохранён! Теперь я смогу давать персональные советы.\n\nЧем займёмся?",
