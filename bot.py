...
def generate_resume_pdf(profile, gpt_resume_text):
    try:
        ...
        name = lines[0] if lines else "Имя не указано"

        story.append(Paragraph(name.upper(), title_style))
        story.append(Paragraph("AI CAREER BOT — карьерный консультант Анастасия", subtitle_style))
        story.append(HRFlowable(width="100%", thickness=2, color=colors.HexColor('#1a237e')))
        story.append(Spacer(1, 0.3*cm))

        sections = gpt_resume_text.split("\n\n")
        for section in sections:
            if not section.strip():
                continue
            lines_s = section.strip().split("\n")
            if lines_s and (lines_s[0].isupper() or lines_s[0].endswith(":")):
                story.append(Paragraph(lines_s[0], section_style))
                story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor('#b0bec5')))
                for line in lines_s[1:]:
                    if line.strip():
        ...
        story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor('#b0bec5')))
        story.append(Paragraph("Создано с помощью AI Career Bot — @anastasia_career_bot", ParagraphStyle(
            'Footer', parent=styles['Normal'], fontSize=8,
            textColor=colors.HexColor('#90a4ae'), alignment=1
        )))
...
2. Каждый раздел начинай с ЗАГЛАВНЫХ БУКВ и двоеточия
...
6. Без лишних слов — только факты
7. Адаптируй под уровень опыта (если нет опыта — акцент на навыки и образование)
"""
...
def build_system_prompt(profile):
    ...
    lang = profile.get("language", "🇷🇺 Русский") if profile else "🇷🇺 Русский"
...
async def callback_handler(update: Update, context):
    ...
    if data == "resume_free":
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "Ты профессиональный составитель резюме. Создавай структурированные профессиональные резюме."},
            ...
            if pdf_path:
                ...
                await q.message.reply_text(
                    ...
                    "👑 VIP резюме — премиум дизайн как на образцах\n"
    ...
    if data == "search_jobs":
        ...
        try:
            ...
            query = target.split("\n")[0] if target else career.split("\n")[-1]
            query = query.replace("Должность:", "").strip()[:50]
            ...
            if not items:
                await q.message.reply_text("По вашему запросу ничего не найдено. Попробуйте изменить параметры.")
                return
    ...
    if data == "hr_simulator":
        ...
        try:
            prompt = "Начни симуляцию HR собеседования. Представься как HR-менеджер компании. Задай первый вопрос для собеседования исходя из профиля кандидата. После каждого ответа давай краткую обратную связь и задавай следующий вопрос из 9 стандартных HR вопросов."
    ...
    if data == "analyze_company":
        await q.message.reply_text(
            "📊 Анализ компании\n\n"
            "Напишите название компании — соберу информацию из открытых источников:\n"
            "• Отзывы сотрудников\n"
            "• Репутация на рынке\n"
    ...
    if data == "find_courses":
        ...
        try:
            prompt = "Подбери 5 конкретных онлайн-курсов для клиента исходя из его профиля, целей и навыков. Укажи: платформу, название, ссылку (если знаешь), стоимость и почему именно этот курс подойдёт."
            reply = ask_gpt(user_id, prompt, profile)
            await q.message.reply_text(reply)
        except Exception as e:
            await q.message.reply_text("Ошибка. Попробуйте ещё раз.")
    ...
    if data == "ask_anastasia":
        await q.message.reply_text(
            "Я слушаю вас! Напишите свой вопрос или расскажите о ситуации 😊\n\n"
