...
def generate_resume_pdf(profile, gpt_resume_text):
    try:
        ...
        c.setFont(FONT, 20)
        c.drawString(margin, height - 42, name_line[:50])
        c.setFont(FONT, 9)
        c.drawString(margin, height - 62, "AI Career Bot — Анастасия, карьерный консультант")
        ...
        for line in lines:
            ...
            is_section = (
                ...
                line.startswith("**") or
                (line[0].isupper() and ":" in line and len(line) < 50)
            )
            ...
            if is_section:
                ...
                c.drawString(margin, y, clean_line[:60])
                y -= 3
                ...
                y = draw_text_wrapped(c, clean, margin, y, content_width, FONT, 10, line_height)

        # Футер
        c.setFillColorRGB(0.56, 0.64, 0.68)
        c.setFont(FONT, 8)
        c.drawCentredString(width / 2, 25, "Создано с помощью AI Career Bot — Анастасия | anastasia-career.ru")
